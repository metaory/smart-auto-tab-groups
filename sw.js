...
const OPT_KEYS = [
	'groupBySubdomain', 'sortAlphabetically', 'ignorePinnedTabs', 'collapseInactive', 'avoidDuplicates',
];
...
const getWin = (id) =>
	(id != null ? Promise.resolve(id) : chrome.windows.getLastFocused().then((w) => w?.id).catch(() => null));
...
const colorFor = (key, idx) => (idx !== undefined ? colorByIndex(idx) : colorForKey(key));

async function createGroup(win, key, tabIds, idx = null) {
	const groupId = await chrome.tabs.group({
		tabIds,
		createProperties: { windowId: win },
	});
	await chrome.tabGroups.update(groupId, { title: key, color: colorFor(key, idx) });
	return { groupId, title: key, key, idx };
}

const groupContainsTab = (groupId, tabId) =>
	chrome.tabs.query({ groupId }).then((tabs) => tabs.some((t) => t.id === tabId));
...
async function collapseOneGroup(g, i, focusedTabId) {
	...
	try {
		await collapseGroupWithMove(g.id, i);
	} catch (err) {
		if (String(err?.message ?? err).includes(TAB_EDIT_BLOCKED)) {
			await delay(80);
			await collapseGroupWithMove(g.id, i).catch(() => {});
		}
	}
	await delay(40);
}
...
async function collapseInactiveInWindow(windowId, activeTabId = null) {
	...
	for (let i = 0; i < groups.length; i++)
		await collapseOneGroup(groups[i], i, focusedTabId);
}
...
async function groupAllTabs(windowId) {
	...
	await delay(300);
	for (const { groupId, key, idx } of groupIds)
		await applyTitleRefresh(groupId, key, idx);
...
async function assignTabToGroup(tabId, url, windowId) {
	...
	if (existing) {
		try {
			const inGroup = await chrome.tabs.query({ groupId: existing.id });
	...
	await createGroup(winId, key, [tabId]);
}
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
	...
	if (msg.action === 'groupNow') groupAllTabs().then(() => r(true)).catch((e) => r(false, e));
	if (msg.action === 'ungroupAll') ungroupAllTabs().then(() => r(true)).catch((e) => r(false, e));
	return true;
});

chrome.commands.onCommand.addListener((cmd) => {
	console.log('[Smart Auto Tab Group] command:', cmd);
	if (cmd === 'group-now') groupAllTabs().catch((e) => console.error('[Smart Auto Tab Group] group-now:', e));
	if (cmd === 'ungroup-all') ungroupAllTabs().catch((e) => console.error('[Smart Auto Tab Group] ungroup-all:', e));
});

chrome.tabs.onCreated.addListener(async (tab) => {
	if (!tab.url?.startsWith('http')) return;
	if (await focusExistingAndCloseNew(tab.windowId, tab.id, tab.url)) return;
	assignTabToGroup(tab.id, tab.url, tab.windowId);
});
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
	if (!changeInfo.url) return;
	if (await focusExistingAndCloseNew(tab.windowId, tabId, changeInfo.url)) return;
	assignTabToGroup(tabId, changeInfo.url, tab.windowId);
});
chrome.tabs.onActivated.addListener(async (info) => {
	await collapseInactiveInWindow(info.windowId, info.tabId);
});
